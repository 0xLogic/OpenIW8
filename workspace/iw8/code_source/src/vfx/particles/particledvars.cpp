...
void __fastcall Particle_RegisterDvars(__int64 a1, __int64 a2, double _XMM2_8)
{
  const dvar_t *v10; 
  const dvar_t *v21; 
  const dvar_t *v39; 
  ...
  v10 = Dvar_RegisterFloat("NNMQOSNQLP", *(float *)&_XMM1, *(float *)&_XMM2, *(float *)&_XMM3, 4u, "Physics memory threshold above which, we don't create fx heavyweight physics");
  __asm { vmovss  xmm1, cs:__real@3f19999a; value }
  particle_physics_memory_threshold = v10;
  __asm
  {
    vmovaps xmm3, xmm8; max
    vxorps  xmm2, xmm2, xmm2; min
  }
  particle_physics_allowedPenetrationDepth = Dvar_RegisterFloat("TMRMQSTK", *(float *)&_XMM1, *(float *)&_XMM2, *(float *)&_XMM3, 4u, "Allowed penetration depth for a heavyweight body when we use particle_physics_doPenetrationTest");
  ...
  particle_debug_draw_systems_distance = Dvar_RegisterFloat("LRLQLNNNML", *(float *)&_XMM1, *(float *)&_XMM2, *(float *)&_XMM3, 4u, "Max distance an effect can be from the camera to draw its info");
  ...
  v21 = Dvar_RegisterFloat("MKRTRSMKPO", *(float *)&_XMM1, *(float *)&_XMM2, *(float *)&_XMM3, 4u, "Used to specify the debug draw length of an emitter's axes");
  __asm { vmovss  xmm3, cs:__real@42c80000; max }
  particle_axes_length = v21;
  __asm
  {
    vxorps  xmm2, xmm2, xmm2; min
    vxorps  xmm1, xmm1, xmm1; value
  }
  particle_edit_mode_loop_time = Dvar_RegisterFloat("SLQNSLTL", *(float *)&_XMM1, *(float *)&_XMM2, *(float *)&_XMM3, 4u, "When editing, keep looping a finite emitter for a number of seconds");
  __asm
  {
    vmovss  xmm3, cs:__real@49742400; max
  ...
  particle_figure8_radius = Dvar_RegisterFloat("NONRSRNOSN", *(float *)&_XMM1, *(float *)&_XMM2, *(float *)&_XMM3, 4u, "Figure 8 radius");
  __asm
  {
    vmovaps xmm3, xmm7; max
    vxorps  xmm2, xmm2, xmm2; min
  ...
  particle_figure8_time = Dvar_RegisterFloat("LPMMNQRQRK", *(float *)&_XMM1, *(float *)&_XMM2, *(float *)&_XMM3, 4u, "Figure 8 time");
  ...
  particle_occlusion_query_override_world_size = Dvar_RegisterFloat("MMPTQTQLKQ", *(float *)&_XMM1, *(float *)&_XMM2, *(float *)&_XMM3, 4u, "Overrides emitter occlusion query world size. -1 restores emitter settings");
  ...
  __asm
  {
    vmovss  xmm1, cs:__real@c7f42400; x
    vmovss  xmm7, cs:__real@7f7fffff
    vmovss  xmm6, cs:__real@ff7fffff
    vmovss  xmm3, cs:__real@c538023d; z
  }
  particle_dump = Dvar_RegisterBool("LLRNSKSLRO", 0, 4u, "Dump particle info");
  __asm
  {
    vmovss  dword ptr [rsp+78h+description], xmm7
    vmovaps xmm2, xmm1; y
    vmovss  [rsp+78h+flags], xmm6
  }
  v39 = Dvar_RegisterVec3("NKPSNPTQMM", *(float *)&_XMM1, *(float *)&_XMM2, *(float *)&_XMM3, flags, description, 0x44u, "Bounding box min of magma heightfield");
  __asm
  {
    vmovss  xmm3, cs:__real@46bd34cd; z
    vmovss  xmm2, cs:__real@47f42400; y
    vmovss  xmm1, cs:__real@47f4240d; x
  }
  particle_magma_heightfield_bbox_min = v39;
  __asm
  {
    vmovss  dword ptr [rsp+78h+description], xmm7
    vmovss  [rsp+78h+flags], xmm6
  }
  particle_magma_heightfield_bbox_max = Dvar_RegisterVec3("MSOOKLOQTQ", *(float *)&_XMM1, *(float *)&_XMM2, *(float *)&_XMM3, flagsa, descriptiona, 0x44u, "Bounding box max of magma heightfield");
